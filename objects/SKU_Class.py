class SKU :
    ...
    def Recipes_containing(self,recipe_book):
        ...
        for i in recipe_book:
            if self.name in recipe_book[i]: #see if the ingredient is in each recipe in the book
                n_recipe +=1
            else: 
                continue
...
#gives instances with the same values as the overall SKU ie Apple
    ...
    def __init__(self, x, name=_sentinel, unit_cost=_sentinel, holding_cost=_sentinel, temp_requirements=_sentinel):

        if name is self._sentinel :

            name = x.name; unit_cost = x.unit_cost; holding_cost = x.holding_cost; temp_requirements = x.temp_requirements
